...
from evaluator_bert import evaluation
...
def train_best(opt, model, train_dataset, dev_info, info_dict):
    ...
    for epoch in range(opt.train_epochs):
        for step, batch_data in enumerate(train_loader):
            ...
            for key in batch_data.keys():
                ...
                # print(key)
            output, loss = model(input_ids=batch_data['input_ids'], attention_mask=batch_data['attention_mask'],
        ...
        tmp_metric_str, tmp_f1, tmp_f1_bio = evaluation(model, dev_info, device)
        ...
        if tmp_f1_bio > max_f1:
            max_f1 = tmp_f1_bio
            max_f1_step = epoch
            save_model(opt, model, type_name='bert')
