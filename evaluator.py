...
def get_base_out(client_model, service_model, loader, device):
    """
    ...
    with torch.no_grad():
        for idx, _batch in enumerate(tqdm(loader, desc=f'Get predict logits')):
            ...
            output = client_model(input_ids=_batch['input_ids'], attention_mask=_batch['attention_mask'],
                                  token_type_ids=_batch['token_type_ids'])[0]
            service_input = output.detach().clone()  # .requires_grad_(True)
            _batch['inputs_embeds'] = service_input.to(device)
            tmp_out = service_model(inputs_embeds=_batch['inputs_embeds'],
                                    attention_mask=_batch['attention_mask'],
                                    token_type_ids=_batch['token_type_ids'])
...
def evaluation(client_model, service_model, dev_info, device):
    """
    线下评估 trigger 模型
    """
    ...
    pred_logits = None
    labels = None
    for tmp_pred, batch_label in get_base_out(client_model, service_model, dev_loader, device):
