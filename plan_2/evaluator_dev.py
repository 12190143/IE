...
def get_base_out(loader):
    ...
    with torch.no_grad():
        for idx, _batch in enumerate(tqdm(loader, desc=f'Get predict logits')):
            ...
            content_data = requests.post(url='http://localhost:5002/client_forward', data=zip_pickle_compress(json_data).getvalue(), headers=headers).content
            json_data = unzip_pickle_compress(content_data)
            content_data = requests.post(url='http://localhost:5003/service_forward', data=zip_pickle_compress(json_data).getvalue(), headers=headers).content
            json_data = unzip_pickle_compress(content_data)
