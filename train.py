...
from model_utils import SeviceModel, ClientModel
...
def train_with_best(opt, info_dict, train_examples, dev_info=None):
...
def training(opt):
    ...
    train_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, '{}_{}_train.json'.format(opt.dataset, opt.bert_type)))
    train_examples = processor.get_train_examples(train_raw_examples, max_seq_len=opt.max_seq_len)

    dev_info = None
    if opt.eval_model:
        dev_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, '{}_{}_dev.json'.format(opt.dataset, opt.bert_type)))
        dev_info = processor.get_dev_examples(dev_raw_examples, max_seq_len=opt.max_seq_len)

    train_with_best(opt, info_dict, train_examples, dev_info)
